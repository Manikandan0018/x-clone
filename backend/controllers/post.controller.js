...
export const getFollowingPosts = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId);
    const following = user.following; // array of ObjectIds

    // Get posts from followed users
    const posts = await Post.find({ user: { $in: following } })
      .sort({ createdAt: -1 })
      .populate('user', 'username img text email'); // Populate user info if needed

    res.status(200).json(posts);
  } catch (error) {
    res.status(500).json({ message: 'Error fetching following posts', error });
  }
};
